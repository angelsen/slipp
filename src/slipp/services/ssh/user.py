...
class UserResolver:
    ...
    def resolve_from_runtime(
        ...
        """Resolve user from runtime info (used by ssh.py).

        This is an alternative interface that takes raw runtime/unit_name
        instead of a Service object.

        Resolution order:
        1. Explicit --user flag
        2. Auto-detect from runtime (root for containers, detected for systemd)
        3. Fallback to default user with warning

        Args:
            runtime: Service runtime (systemd, docker, podman)
            unit_name: Systemd unit name (if applicable)
            explicit_user: User from --user flag (if provided)
            default_user: Fallback user (typically SSH user)
        ...
        if runtime and Runtime(runtime).is_container():
            return UserResolution(user="root", source="detected")

        if runtime and unit_name:
            detected = self.get_service_user(unit_name)
            if detected:
                return UserResolution(user=detected, source="detected")
            ...
            return UserResolution(
                user=default_user,
                source="default",
                warning=f"Could not detect user for service, using {default_user}. "
                "Hint: Use -u <user> to specify explicitly",
            )
