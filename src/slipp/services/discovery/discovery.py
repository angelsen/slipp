...
the discover → enrich pipeline (single-host and multi-host) used by commands.
See services/discovery/filtering.py for filtering/lookup over already-discovered
services, and services/status for `systemctl status` output parsing.
"""
...
def discover_and_enrich(
    ...
    services = discovery.enrich_with_projects(services, host_to_projects)
    return services
