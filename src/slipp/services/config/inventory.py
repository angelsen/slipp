...
def _write_inventory(path: Path, inventory: InventoryConfig) -> None:
    """Write inventory.yml through the same marked template InventoryFileStage uses.

    Every slipp-owned writer of this file must produce output carrying the
    `# Generated by slipp launch` marker -- a write that strips it (e.g. a
    raw yaml.dump()) makes is_customized() treat the file as hand-edited
    forever after, silently disabling InventoryFileStage's regeneration
    (including ProxyResolutionStage's proxy_owner cache write) on every
    future launch for that project, with no error, just a skip.
    """
    atomic_write_text(path, generate_inventory(inventory))
