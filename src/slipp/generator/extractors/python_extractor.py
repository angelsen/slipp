...
from slipp.scanner.helpers import detect_python_dep_manager, load_pyproject


def extract_python(service: DetectedService) -> dict[str, Any]:
    ...
    if variables["streamlit"]:
        variables["entrypoint"] = _find_streamlit_entrypoint(service.path)

    # Systemd ExecStart resolution (native, non-container deploys)
    variables.update(_find_exec_entrypoint(service.path))
...
def _detect_python_version(service_path: Path) -> str:
...
def _detect_dependency_manager(service_path: Path) -> dict[str, bool]:
...
def _find_streamlit_entrypoint(service_path: Path) -> str:
...
def _find_exec_entrypoint(service_path: Path) -> dict[str, Any]:
    ...
    """
    data = load_pyproject(service_path)
    if data is not None:
