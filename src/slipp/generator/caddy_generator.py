...
def generate_caddy_role(
    config: CaddyConfig, project_name: str, app_domain: str, admin_email: str
) -> dict[Path, str]:
    ...
    Example:
        ...
        >>> files = generate_caddy_role(config, "myapp", "example.com", "admin@example.com")
    """
    return {
        Path("roles/caddy/tasks/main.yml"): _render_tasks(config, project_name),
        Path("roles/caddy/templates/Caddyfile.j2"): _render_main_caddyfile(
...
def _render_tasks(config: CaddyConfig, project_name: str) -> str:
...
def _render_main_caddyfile(
    config: CaddyConfig, app_domain: str, admin_email: str
) -> str:
...
def _render_site_template(config: CaddyConfig, project_name: str) -> str:
...
def _render_defaults(config: CaddyConfig) -> str:
