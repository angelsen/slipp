...
The legacy `.slipp/runs.yaml` is still read (with a deprecation warning) if
present — move its entries into `slipp.yaml` under `runs:`.
...
Generate only Dockerfiles for detected services. Use `--runtime docker|podman`
(default: `docker`) to select the target container runtime.
